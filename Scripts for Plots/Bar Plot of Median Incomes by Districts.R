...
bar.plt.soc.group.i <- function(x, main = NULL){
  ...
  bar.plt.script <- barplot(v, las = 1, ylim = c(min(v) - 10, max(v) + 10),
  ...
  legend("topright", legend=c((sprintf("Mean:    US$ %s",round(mean(v),2))),
                              sprintf("Median: US$ %s", round(median(v),2))),
         fill = c("red", "green"), cex = .8, bty = "n")
  
  par(mar = c(4, 4, 4, 4)) # Define borders of the plot
  
  axis(side = 2, at = p.seq, las = 1) # Axes
  axis(side = 4, at = p.seq, las = 1)
  
  box() # Put Bar Plot into box
}
bar.plt.soc.group.i(rus.bubble.df1,
                    main =
                      "Bar Plot of Median Incomes across Federal Districts") 
