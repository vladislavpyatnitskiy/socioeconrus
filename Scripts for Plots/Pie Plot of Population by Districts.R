pie.plt.soc.group <- function(x, main = NULL){ # Population by districts
  ...
  pie(pct[,2], labels = c(sprintf("%s %s%%", pct[,1], pct[,2])), col = colors37,
      main = main, radius = 1.75) # Plot
}
pie.plt.soc.group(rus.bubble.df1,
                  main = "Russian Population by Federal Districts") # Test
