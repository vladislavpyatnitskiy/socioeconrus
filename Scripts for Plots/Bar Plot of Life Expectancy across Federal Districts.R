# Life Expectancy across Federal Districts
bar.plt.soc.group.y <- function(x, main = NULL){
  
  pct <- data.frame(x[,2], as.numeric(x[,4])) # Create Separate Data Frame
  
  colnames(pct) <- c("Federal District", "Life Expectancy") # Column names
  
  # Aggregate life expectancy data to get median values for each district
  pct <- aggregate(`Life Expectancy` ~ `Federal District`, data=pct, median)
  ...
  bar.plt.script <- barplot(v, las = 1, ylim = c(min(v) - 1, max(v) + 1),
...
bar.plt.soc.group.y(rus.bubble.df1,
                    main =
