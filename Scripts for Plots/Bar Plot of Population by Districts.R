bar.plt.soc.group <- function(x){ # Bar Plot of Population by Districts
  ...
  options(scipen = 1000000) # Change format to numeric
  
  pct <- data.frame(x[,2], x[,5]) # Federal District and Population data
  
  colnames(pct) <- c("Federal District", "Population") # Assign column names
  
  pct <- aggregate(Population ~ `Federal District`, data=pct, sum) # Sum
  
  regions <- pct[,1] # Make Federal District names as row names
  
  pct <- pct[,-1] # Reduce excessive names 
  
  names(pct) <- regions # Put names 
  
  pct <- sort(pct, decreasing = T) / 1000000 # Sort values
  
  p.seq <- seq(0, max(pct), round(max(pct), -1) / 8) # Set axes values
  ...
  # Bar Plot
  B <- barplot(pct, ylim = c(0, max(pct) + 1), names.arg = names(pct), col = C,
               las=1, main = "Population among Federal Districts in millions")
  ...
  legend("bottom",xpd=T,legend=c((sprintf("Mean: %s millions",
                                          round(mean(pct),2))),
                                 sprintf("Median: %s millions",
                                         round(median(pct),2))),
...
bar.plt.soc.group(rus.bubble.df1) # Test
