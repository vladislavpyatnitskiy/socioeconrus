...
dif.life.exp.ru <- function(){ # Male & Female Life Expectancies in Russia
  
  x = "List_of_federal_subjects_of_Russia_by_life_expectancy"
  ...
  v <- data.frame(
    y[seq(from = 1, to = length(y), by = 20)], # Region name
  ...
  for (n in 1:nrow(v)) if (isTRUE(grepl("Oblast", v[n,1]))){ # Reduce "Oblast"
    ...
    O <- read.fwf(
      textConnection(v[n,1]), widths = c(nchar(v[n,1]) - 7, 1),
